...
RsNPL2 = np.hstack([C2[:, 0 ,0].reshape(Num_neuron, 1), C2[:, 1:, 0], C2[:, 1:, 1], C2[:, 1:, 2]]) ##### Different from matlab
...
for i in range(1, len(t)):
    Ro2[:, i] = np.squeeze(np.tanh(A2 @ Ro2[:, i-1].reshape(Num_neuron, 1) + B2 @ X2[i-1].reshape(1, 1) + d2))
