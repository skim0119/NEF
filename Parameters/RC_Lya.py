...
class ReservoirComputingWithLE:
    def __init__(self, num_neuron=300, spectral_radius=1.2, sigma=0.1, sparsity=0.98, beta=0.0):
        ...
        self.sigma = sigma
        self.beta = beta
        self.Win = np.random.uniform(-sigma, sigma, (num_neuron, 3))  # Input weight matrix
        self.recurrent_matrix = np.random.rand(num_neuron, num_neuron) - 0.5  # Reservoir recurrent weight matrix
        self.recurrent_matrix[np.random.rand(*self.recurrent_matrix.shape) < sparsity] = 0  # Apply sparsity
        rho_A = max(abs(np.linalg.eigvals(self.recurrent_matrix)))  # Compute spectral radius
...
initial_input = data_train[-1]  # Use the last state from training data as the initial input for prediction
...
lce_values, history = LCE(RC_DS, 3, 0, 10**4, True)  # Use 10^4 steps to compute LCE for more accurate results
