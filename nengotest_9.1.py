...
class NengoReservoirComputing:
    def __init__(
        ...
        with self.model:
            ...
            self.reservoir = nengo.Ensemble(
                n_neurons=num_neuron, dimensions=num_neuron, neuron_type=nengo.Direct()
            )

            # Connect input to reservoir
            nengo.Connection(
                self.input, self.reservoir, transform=self.Win, synapse=None
            )
            ...
            nengo.Connection(
                self.reservoir,
                self.reservoir,
                transform=self.recurrent_matrix,
                synapse=0,
                function=np.tanh,
            )
    ...
    def train(self, inputs, dt=0.02):
        ...
        training_loss = mean_squared_error(
            inputs[1:], self.Lmodel.predict(reservoir_states)[1:]
        )
    ...
    def prediction(self, steps, dt=0.02):
        ...
        with self.model:
            ...
            nengo.Connection(
                self.reservoir, self.input, transform=self.Lmodel.coef_, synapse=0
            )
