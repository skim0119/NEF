...
def random_geometric(n, p, mean, std):
    ...
    for (old_weight, old_dist, u, v), new_weight in zip(
        weight_list[:k], gaussian_weights
    ):
        graph[u][v]["weight"] = new_weight
...
def compute_controllability(graph_type, num_nodes, num_iter):
    ...
    for iter in range(num_iter):
        ...
        eigenvalues_adjacent_re, eigenvectors_adjacent_re = np.linalg.eig(
            mat_adjacent_re
        )
        ...
        for node in range(num_nodes):
            ...
            modal_controllability[iter, node] = phi
...
WRG_glo, WRG_ave, WRG_mod = compute_controllability(
    graph_type="WRG", num_nodes=num_nodes, num_iter=num_iter
)
RL_glo, RL_ave, RL_mod = compute_controllability(
    graph_type="WRG", num_nodes=num_nodes, num_iter=num_iter
)
WS_glo, WS_ave, WS_mod = compute_controllability(
    graph_type="WRG", num_nodes=num_nodes, num_iter=num_iter
)
RG_glo, RG_ave, RG_mod = compute_controllability(
    graph_type="WRG", num_nodes=num_nodes, num_iter=num_iter
)
BA_glo, BA_ave, BA_mod = compute_controllability(
    graph_type="WRG", num_nodes=num_nodes, num_iter=num_iter
)
